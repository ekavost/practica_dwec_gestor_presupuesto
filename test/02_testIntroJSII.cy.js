...
describe("Función CrearGasto y funcionamiento de objeto gasto", function () {
  it("CrearGasto: comprobación con fecha y etiquetas", function () {
    ...
    let gasto7 = new CrearGasto(descr, 23.55, "2021-10-06T13:10Z", "casa", "supermercado", "casa");
    assert.equal(gasto7.descripcion, descr);
    assert.equal(gasto7.valor, 23.55);
    assert.equal(gasto7.fecha, Date.parse("2021-10-06T13:10Z"));
    assert.lengthOf(gasto7.etiquetas, 2);
    assert.equal(gasto7.etiquetas[0], "casa");
    assert.equal(gasto7.etiquetas[1], "supermercado");
  });
